...
def do_test(h, s, __expected):
    startTime = time.time()
    instance = LiveConcert()
    exception = None
    try:
        __result = instance.maxHappiness(h, s);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("LiveConcert (250 Points)\n\n")
    ...
    with open("LiveConcert.sample", "r") as f:
        while True:
            label = f.readline()
            ...
            passed += do_test(h, s, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1444964625
    PT, TT = (T / 60.0, 75.0)
