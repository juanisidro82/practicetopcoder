...
def do_test(duracion, __expected):
    startTime = time.time()
    instance = ExerciseMachine()
    exception = None
    try:
        __result = instance.getPercentages(duracion);
    except:
        import traceback
...
def run_tests():
    ...
    with open("ExerciseMachine.sample", "r") as f:
        while True:
            ...
            passed += do_test(duracion, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1442929703
    PT, TT = (T / 60.0, 75.0)
