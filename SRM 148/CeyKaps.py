...
def do_test(typed, switches, __expected):
    startTime = time.time()
    instance = CeyKaps()
    exception = None
    try:
...
def run_tests():
    sys.stdout.write("CeyKaps (600 Points)\n\n")
    ...
    with open("CeyKaps.sample", "r") as f:
        while True:
            label = f.readline()
            if not label.startswith("--"): break

            typed = f.readline().rstrip()
            switches = []
            for i in range(0, int(f.readline())):
                switches.append(f.readline().rstrip())
            ...
            passed += do_test(typed, switches, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1444584112
    PT, TT = (T / 60.0, 75.0)
