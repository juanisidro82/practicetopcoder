...
class SubstitutionCipher:
    def decode(self, a, b, y):
        tablasustitucion = dict()
        for i in range(len(a)):
            tablasustitucion[b[i]] = a[i]
        if len(tablasustitucion) == 25:
            b0 = (set(ABECEDARIO) - set(b)).pop()
            a0 = (set(ABECEDARIO) - set(a)).pop()
            tablasustitucion[b0] = a0
        respuesta = ""
        for i in range(len(y)):
            if not y[i] in tablasustitucion.keys():
                return ""
            respuesta += tablasustitucion[y[i]]
        return respuesta
